	...
	public void parse(Map<String, String> map) {
		if (map.get(Constants.VERTEX_ID) != null) {
			...
			if (initRequestedTime == 0) {
				initRequestedTime = Long
						.parseLong(map.get(Constants.INIT_REQUESTED_TIME) == null ? "0"
								: map.get(Constants.INIT_REQUESTED_TIME));
			}
			if (initedTime == 0) {
				initedTime = Long
						.parseLong(map.get(Constants.INITED_TIME) == null ? "0"
								: map.get(Constants.INITED_TIME));
			}
			if (numTasks == 0) {
				numTasks = Long
						.parseLong(map.get(Constants.VERTEX_TASKS) == null ? "0"
								: map.get(Constants.VERTEX_TASKS));
			}
			if (startRequestedTime == 0) {
				startRequestedTime = Long
						.parseLong(map.get(Constants.START_REQUESTED_TIME) == null ? "0"
								: map.get(Constants.START_REQUESTED_TIME));
			}
			if (startedTime == 0) {
				startedTime = Long
						.parseLong(map.get(Constants.STARTED_TIME) == null ? "0"
								: map.get(Constants.STARTED_TIME));
			}
			if (finishTime == 0) {
				finishTime = Long
						.parseLong(map.get(Constants.FINISH_TIME) == null ? "0"
								: map.get(Constants.FINISH_TIME));
			}
			if (timeTaken == 0) {
				timeTaken = Long
						.parseLong(map.get(Constants.TIME_TAKEN) == null ? "0"
